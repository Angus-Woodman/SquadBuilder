...
def _friendship_to_dict(f: Friendship, perspective_user_id: int) -> dict[str, Any]:
    """Serialize a Friendship from one user's perspective."""
...
def list_friends(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return [_friendship_to_dict(f, user.id) for f in friendships]
